...
/// <summary>
///   Wraps T-SQL batches in error-handling code to improve diagnostic messages.
/// </summary>
public static class SqlErrorHandling
{
    /// <summary>
    ///   Wraps the specified T-SQL batches with error-handling code, yielding
    ///   a single combined batch.
    /// </summary>
    /// <param name="batches">
    ///   The batches to wrap.
    /// </param>
    /// <returns>
    ///   A T-SQL batch that performs <paramref name="batches"/> and yields
    ///   improved diagnostic messages in the case of errors.
    /// </returns>
    ...
    private static readonly Regex NoWrapRegex = new(
        @"^--#[ \t]*NOWRAP[ \t]*\r?$",
        Options
    );
